        ...
        for p in range(2):
            for i in range(1, 4):
                if p == 0:
                    ...
                    lis = state.buffer
                if len(lis) < i:
                    word_list.append(self.word_vocab['<NULL>'])
                else:
                    word_idx = lis[-1*i]
                    word = words[word_idx]

                    if word is None:
                        word_list.append(self.word_vocab['<ROOT>'])
                    else:
                        if pos[word_idx] == 'CD':
                            word_list.append(self.word_vocab['<CD>'])
                        elif pos[word_idx] == 'NNP':
                            word_list.append(self.word_vocab['<NNP>'])
                        else:
                            word = word.lower()
                            ...
                            if word in self.word_vocab:
                                ...
                                word_list.append(self.word_vocab['<UNK>'])

        return np.array(word_list, dtype='float32')

    def get_output_representation(self, output_pair):  
        # TODO: Write this method for Part 2
        trans_pair_idx = self.output_labels[output_pair]
        onehot = np.zeros(len(self.output_labels), dtype='float32')

        # Change the value to 1 only for the input transition index
        onehot[trans_pair_idx] = 1
